...
The source code for this tutorial is in the operator_panel
...
Here is what RViz looks like with the new "Wasp" panel showing on
the left:
...
The code for WaspPanel is in these files: 
...
Add a Wasp panel by opening the "Panels" menu and then "Add New
Panel" within that.  This should bring up a Panel class chooser dialog
with "Wasp" in it (here it is "operator_panel"):

.. image:: teleop_plugin.png

If "Wasp" is not in your list of Panel types, look through RViz's
...
- not properly referencing the library file (like
  liboperator_panel.so) from plugin_description.xml.

Once you've added the Wasp panel to RViz, you just need to enter a
...
This Wasp panel might be useful as it is, since it already sends out
